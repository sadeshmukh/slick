(function () {
  ...
  const rng = cr && cr.getRandomValues
    ? n => { let a = new Uint32Array(n); cr.getRandomValues(a); return a; }
    : n => { let a = new Uint32Array(n); for (let i = 0; i < n; i++) a[i] = (Math.random() * 0xffffffff) >>> 0; return a; };
  ...
  const ext = n => {
    ...
    let b = n.slice(Math.max(n.lastIndexOf('/'), n.lastIndexOf('\\')) + 1), dot = b.lastIndexOf('.');
  ...
  Object.defineProperty(File.prototype, 'name', {
    ...
    get: function () {
      let r;
      try { r = og.call(this); } catch (e) { return og.call(this); }
      let v = c.get(this);
