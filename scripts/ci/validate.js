...
for (const f of themes) {
  ...
  for (const [ramp, shades] of Object.entries(t.palette || {}))
    for (const [shade, v] of Object.entries(shades))
      if (!/^\d{1,3},\d{1,3},\d{1,3}$/.test(String(v))) fail(f, `palette.${ramp}.${shade} must be raw "r,g,b", got: ${v}`);
  ...
  try { buildSpec(p); } catch (e) { fail(f, `buildSpec threw: ${e.message}`); }
}
...
for (const d of dirs) {
  let m;
  try { m = require(path.join(PLUGINS, d, 'index.js')); }
  catch (e) { fail(`plugins/${d}`, `index.js failed to load: ${e.message}`); continue; }
...
} catch (e) { fail('plugins/enabled.json', e.message); }
